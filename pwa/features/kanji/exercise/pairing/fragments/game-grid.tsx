...
export function GameGrid() {
  ...
  const shuffledMeanings = useMemo(() => {
    const meanings = gameWords.map((w: any) => isIndonesian ? w.meaning_id : w.meaning_en);
  ...
  const handleCardClick = (id: string, type: "kanji" | "meaning") => {
    ...
    if (selectedCards.length === 0) {
      ...
      if (firstCard.type !== type) {
        ...
        if (matchingWord) {
          // Correct match
          const newMatchedPairs = new Set([...matchedPairs, kanjiCard.id, meaningCard.id]);
        ...
        {shuffledKanji.map((kanji: string) => {
          ...
          return (
            <PairingCard
              ...
              type="kanji"
              isSelected={selectedCards.some((c: SelectedCard) => c.id === kanji)}
      ...
      <div className="space-y-3">
        {shuffledMeanings.map((meaning: string, index: number) => (
          <PairingCard
            ...
            content={meaning}
            type="meaning"
            isSelected={selectedCards.some((c: SelectedCard) => c.id === meaning)}
