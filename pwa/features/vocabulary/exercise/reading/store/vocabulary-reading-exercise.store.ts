...
import {
  ...
  calculateScore,
  checkAnswer,
} from "../utils/vocabulary-reading.utils";
...
export const useVocabularyReadingExerciseStore =
  create<VocabularyReadingExerciseState>((set, get) => ({
    // Semantic State Groups
    gameState: {
      questions: [],
      correctQuestions: [],
    ...
    getSelectedAnswerText: () => {
      const {
        questionState: { selectedOption, directInput, inputMode },
      } = get();
    ...
    startRetryMode: () => {
      const {
        gameState: { wrongQuestions, score, questions },
      } = get();
    ...
    initializeExercise: async (
      ...
      try {
        ...
        console.error(
          "Failed to initialize vocabulary reading exercise:",
          error
        );
      }
    },
