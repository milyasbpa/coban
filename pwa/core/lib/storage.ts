...
export class StorageManager {
  ...
  static async createDefaultUserScore(userId: string, level: "N5" | "N4" | "N3" | "N2" | "N1" = "N5"): Promise<UserScore> {
  ...
  static async getExerciseAttempt(attemptId: string): Promise<ExerciseAttempt | null> {
    return await exerciseStorage.getItem(attemptId);
  }
  
  static async getRecentAttempts(limit: number = 10): Promise<ExerciseAttempt[]> {
  ...
  static async getAttemptsByLesson(lessonId: string): Promise<ExerciseAttempt[]> {
    const attempts: ExerciseAttempt[] = [];
  ...
  static async getAttemptsByExerciseType(exerciseType: "writing" | "reading" | "pairing"): Promise<ExerciseAttempt[]> {
    const attempts: ExerciseAttempt[] = [];
  ...
  static async saveDailyProgress(userId: string, date: string, progress: DailyProgress): Promise<void> {
    const key = `${userId}_daily_${date}`;
    await analyticsStorage.setItem(key, progress);
  }
  
  static async getDailyProgress(userId: string, startDate: string, endDate: string): Promise<DailyProgress[]> {
  ...
  static async saveWeeklyProgress(userId: string, weekStart: string, progress: WeeklyProgress): Promise<void> {
    const key = `${userId}_weekly_${weekStart}`;
    await analyticsStorage.setItem(key, progress);
  }
  
  static async getWeeklyProgress(userId: string, limit: number = 12): Promise<WeeklyProgress[]> {
