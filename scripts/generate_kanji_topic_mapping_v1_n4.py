...
SRC = ROOT / "data" / "n4" / "kanji" / "kanji.json"
OUT = ROOT / "data" / "n4" / "kanji" / "kanji_topic_mapping_v1.json"

"""
Refined taxonomy without generic misc buckets. Each bucket contains:
- key: snake_case
...
BUCKETS = [
    ...
    {"key": "counters_and_quantifiers", "name": "Counters & Quantifiers", "description": "Counters and quantifiers.", "keywords": [
        "counter", "book", "sheet", "cup", "animal"
    ], "priority": 8},
    {"key": "time_sequence", "name": "Time & Sequence", "description": "Time, order, sequence.", "keywords": [
        "end", "future", "next", "sequence", "row", "line", "order", "period", "term", "finish", "instant"
    ], "priority": 9},
    {"key": "space_direction_location", "name": "Space, Direction & Location", "description": "Space, direction, places, facilities.", "keywords": [
        "inside", "direction", "face", "place", "location", "surface", "station", "stop", "line", "route", "road", "corner", "beach", "river", "hill", "cave"
    ], "priority": 10},
    {"key": "movement_travel_transport", "name": "Movement, Travel & Transport", "description": "Move, pass, travel, transport.", "keywords": [
        "chase", "escape", "pass", "cross", "connect", "travel", "ship", "boat", "stop", "road", "bridge", "airport", "round trip"
    ], "priority": 11},
    {"key": "actions_operations", "name": "Actions & Operations", "description": "Common verbs and operations.", "keywords": [
        "hit", "strike", "pull", "push", "add", "reduce", "return", "welcome", "prevent", "protect", "attach", "stick", "fold", "turn", "rotate", "throw", "bring", "take", "collect", "save", "compare", "pay", "receive", "give", "use", "mix", "cut", "open", "close", "replace", "transmit", "attack", "refuse", "worship", "pray", "clap", "carry", "enlarge", "contain", "separate", "suck", "vomit", "shout"
    ], "priority": 12},
    {"key": "change_and_transformation", "name": "Change & Transformation", "description": "Change, modify, reform, replace.", "keywords": [
        "change", "modify", "reform", "decide", "replace", "increase", "decrease", "mix", "break", "stop", "instantly"
    ], "priority": 13},
    {"key": "states_conditions", "name": "States & Conditions", "description": "States, existence, necessity, completion.", "keywords": [
        "necessary", "must", "complete", "completion", "exist", "existence", "lack", "lose", "available", "present", "flat", "peaceful", "low"
    ], "priority": 14},
    {"key": "comfort_risk_safety", "name": "Comfort, Risk & Safety", "description": "Comfort, discomfort, danger, safety.", "keywords": [
        "comfortable", "trouble", "danger", "dangerous", "safe", "cold", "hot", "hardship"
    ], "priority": 15},
    {"key": "cleanliness_quality_intensity", "name": "Cleanliness, Quality & Intensity", "description": "Clean/dirty, light/dark, thick/thin.", "keywords": [
        "dirty", "clean", "thick", "thin", "dark", "bright", "strong", "weak", "rare", "shape", "form", "sweet", "spicy"
    ], "priority": 16},
    {"key": "body_senses_self", "name": "Body, Senses & Self", "description": "Body parts, senses, self.", "keywords": [
        "body", "self", "ear", "blood", "nose", "hair", "skin", "tongue"
    ], "priority": 17},
    {"key": "health_medicine", "name": "Health & Medicine", "description": "Health, symptoms, treatment.", "keywords": [
        "disease", "illness", "pain", "ache", "fever", "medicine", "doctor", "nurse", "clinic", "diagnose", "therapy", "treat", "heal", "symptom", "pregnancy"
    ], "priority": 18},
    ...
    {"key": "communication_expression", "name": "Communication & Expression", "description": "Say, report, request, warn, announce.", "keywords": [
        "say", "speak", "report", "announce", "warn", "request", "ask", "call", "summon", "show", "indicate", "suggest", "complaint"
    ], "priority": 20},
    ...
    {"key": "technology_tools_devices", "name": "Technology, Tools & Devices", "description": "Tools, devices, equipment.", "keywords": [
        "tool", "device", "equipment", "machine", "gear", "engine", "install", "setup", "light", "lighthouse", "mark", "printing"
    ], "priority": 22},
    {"key": "manufacturing_industry_production", "name": "Manufacturing, Industry & Production", "description": "Make/build, production, factories.", "keywords": [
        "manufacture", "produce", "factory", "build", "create", "make", "construct", "assemble", "canned"
    ], "priority": 23},
    {"key": "materials_substances", "name": "Materials & Substances", "description": "Materials, substances, resources.", "keywords": [
        "stone", "wood", "metal", "water", "oil", "sand", "salt", "iron", "powder", "bone", "material", "ice", "ash", "dust"
    ], "priority": 24},
    {"key": "nature_environment_animals_plants", "name": "Nature, Environment, Animals & Plants", "description": "Natural world, animals, plants, environment.", "keywords": [
        "insect", "cat", "grass", "environment", "condition", "outside", "field", "sheep", "wool", "wheat", "cedar", "village", "ranch"
    ], "priority": 25},
    ...
    {"key": "transportation_routes_infrastructure", "name": "Transportation, Routes & Infrastructure", "description": "Roads, routes, stops, bridges.", "keywords": [
        "road", "route", "stop", "bridge", "line", "city block"
    ], "priority": 41},
    ...
    {"key": "assistance_support_help", "name": "Assistance, Support & Help", "description": "Help, assistance, support.", "keywords": [
        "support", "assist", "aid", "help", "respond"
    ], "priority": 43},
...
def choose_bucket(text: str) -> dict | None:
    t = text.lower()
...
def main():
    ...
    for it in items:
        ...
        if not bucket:
            if any(k in (en or "").lower() for k in ["complete", "completion", "cause", "reason", "plan", "exist", "freedom", "become", "order"]):
                bucket = next(b for b in BUCKETS_SORTED if b["key"] == "abstract_logic_reasoning")
            elif any(k in (en or "").lower() for k in ["hit", "pull", "add", "return", "prevent", "attach", "fold", "turn", "throw", "collect", "pay", "compare", "mix", "replace", "cut", "open", "close", "attack", "refuse", "worship", "pray", "clap", "carry", "enlarge", "contain", "separate", "suck", "vomit", "shout"]):
                bucket = next(b for b in BUCKETS_SORTED if b["key"] == "actions_operations")
            elif any(k in (en or "").lower() for k in ["necessary", "busy", "comfortable", "trouble", "exist", "lack", "lose", "dangerous", "cold", "dirty", "rare", "win", "failure", "low"]):
                bucket = next(b for b in BUCKETS_SORTED if b["key"] == "states_conditions")
            elif any(k in (en or "").lower() for k in ["ticket", "line", "stop", "road", "bridge", "route", "travel", "ship", "airport", "round trip"]):
                # pick one of transportation-like themes prioritizing movement
                for k in ["movement_travel_transport", "transportation_routes_infrastructure", "travel_documents_tickets"]:
                    b = next((b for b in BUCKETS_SORTED if b["key"] == k), None)
                    if b:
                        bucket = b
                        break
            else:
    ...
    # Final uniqueness and fill missing into a specific neutral bucket
    used_ids = set(i for v in topic_categories.values() for i in v["kanji_ids"]) 
    if len(used_ids) < total_items:
        neutral_key = "states_conditions"
        base_name = next(b["name"] for b in BUCKETS_SORTED if b["key"] == neutral_key)
        desc = next(b["description"] for b in BUCKETS_SORTED if b["key"] == neutral_key)
        missing = []
        for it in items:
            if it["id"] not in used_ids:
                missing.append((it["id"], it["character"]))
        chunks = [missing[i:i+10] for i in range(0, len(missing), 10)]
        for idx, chunk in enumerate(chunks, start=1):
            sub_key = neutral_key if idx == 1 else f"{neutral_key}_{idx}"
            sub_name = base_name if idx == 1 else f"{base_name} {idx}"
            topic_categories[sub_key] = {
                "name": sub_name,
                "kanji_ids": [i for i, _ in chunk],
                "kanji_characters": [c for _, c in chunk],
            ...
            used_ids.update([i for i, _ in chunk])
    ...
    summary = {
        ...
        "description": "All N4 kanji clustered by refined topics (no generic buckets), max 10 per group.",
    }

    output = {"topic_categories": {k: topic_categories[k] for k in sorted(topic_categories.keys())}, "summary": summary}
